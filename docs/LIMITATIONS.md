...
| **Annual renewal reminders** | Built: `send-renewal-reminders` emails 30 days before a yearly plan renews, stating that it renews, when, for how much and how to cancel, once per period. It needs `EMAIL_PROVIDER` configured to actually send; until then the rows queue in `jobs`. Confirm the wording against the states you sell in before enabling yearly prices. |
